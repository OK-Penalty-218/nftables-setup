...
confirm_run() {
    read -p "WARNING! Do you want to run this script? This action will set the firewall to ACCEPT ALL inbound/outboung/fowarded traffic. (y/n): " choice
...
echo "Resetting all firewall settings."
...
# Define chains
$nft add chain inet firewall inbound { type filter hook input priority 0 \; policy accept \; }
$nft add chain inet firewall outbound { type filter hook forward priority 0 \; policy accept \; }
...
echo "!!!WARNING!!!"
echo "Firewall set to allow all inbound/outbound/fowarded traffic."
echo "!!!WARNING!!!"
